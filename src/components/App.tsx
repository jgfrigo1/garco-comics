...
import { loadBookmarks, saveBookmarkLocal, loadConfig } from './services/mockDataService'; // We still use these for now
import AuthView from './components/AuthView';
import LibraryView from './components/LibraryView';
import ReaderView from './components/ReaderView';
import SpideyChat from './components/SpideyChat';
import { Menu, X, Library, Globe, LogOut, ChevronDown, ChevronRight, Layers } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      try {
        // Fetch comics from the local JSON file
        const response = await fetch('/data/comics.json'); // Fetches from the public folder
        if (!response.ok) {
    ...
    <div className="h-screen w-screen font-sans flex flex-col md:flex-row bg-spidey-black overflow-hidden border-4 border-black">
      ...
      <aside className={`fixed md:sticky top-0 left-0 h-full w-72 bg-spidey-red text-white z-50 transition-transform duration-300 transform border-r-4 border-black shadow-2xl flex flex-col shrink-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="p-4 space-y-4 relative z-10 flex-1 overflow-y-auto scrollbar-hide bg-spidey-red">
            ...
            {/* The sync button is now removed as we load locally */}
        ...
        <div className="p-4 border-t-4 border-black bg-black/10 shrink-0">
             <button onClick={() => setState(prev => ({...prev, isAuthenticated: false, passwordInput: ''}))} className="flex items-center gap-2 text-white/70 hover:text-white transition-colors w-full px-2 font-bold uppercase text-xs">
